...
const GROUP_LT: Record<CheckResult['group'], string> = {
  geometry: 'Geometrinis patikrinimas',
  logic: 'Logikos patikrinimas',
  completeness: 'Pilnumo patikrinimas',
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <h3 className="text-lg font-semibold">Savikontrolės rezultatai</h3>
        <span className={
          warns === 0
          ...
          {warns === 0 ? 'Viskas tvarkoje' : `${warns} įspėjimai`}
        </span>
      </div>
      {groups.map((g) => {
        ...
        return (
          <div key={g}>
            <p className="mb-1.5 text-sm font-medium text-muted-foreground">{GROUP_LT[g]}</p>
